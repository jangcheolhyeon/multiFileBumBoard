            ...
            <table class="boardWrite no_line">
                ...
                <tbody>
                    <tr>
                        ...
                        <th scope="row">연락처</th>
                        <td>
                            <input type="text" class="form" placeholder="전화번호는 -을 넣어주세요 ex)010-1234-5678" value="${boardInfo.writer_phone }" id="writePhone" title="텍스트 입력" name="writer_phone">
...
<!--                             <div class="phone-number">
                    ...
                    <tr>                                
                        ...
                        <td>
                        	<!-- modify할때 -->
                        	<c:if test="${modifyState == 1}">
	                   	        <select class="form wp45" id="selectType1" name="type1" onchange="onChangeType1()">
	                            ...
	                            <select class="form wp45" id="selectType2" name="type2">
	                                <option>선택하세요</option>
	                            </select>     
                        	</c:if>
                    ...
                    <tr>
                        <th scope="row">내용</th>
                        <td colspan="3">
                            <textarea class="form" title="텍스트 입력" name="content" id="textContent">${boardInfo.content}</textarea>
                        </td>
                    </tr>
                    <tr class="file-tr">
...
<!--                             <button class="btn-txt form-fileAdd"><i class="fas fa-plus"></i> 파일추가</button> -->
                            ...
                            <div class="filebox">
                                <input type="file" id="uploadFile" name="uploadFile" multiple="multiple" class="upload-name" value="첨부파일" placeholder="첨부파일">
                    ...
                    <tr>
                        ...
                        <td colspan="3">
                            <input type="password" class="form wp50" id="board_password" name="board_password" value="${boardInfo.board_password}" placeholder="텍스트 입력" title="비밀번호 입력">
                            <p class="import"><span><i class="fa fa-exclamation" aria-hidden="true"></i></span> 비밀번호는 4자 이상 적어주세요</p>
                        </td>
                    </tr>
                </tbody>
            </table>
                ...
                <div class="btnWrap">
	                <div class="fr">
	                	...
	                	<c:if test="${empty param.board_id}">
		                    ...
		                    <a href="#this" id="writeBtn" class="btn btn-primary" onclick="onClickWriteBtn()"><i class="fas fa-check"></i> 저장</a>
	                	</c:if>
	                	
	                	<!-- modify할때 -->
	                	<c:if test="${not empty param.board_id}">
		                    <a href="<c:url value="readBoard.do?board_id="/>${boardInfo.board_id}" class="btn btn-danger">취소</a>
		                    <a href="#this" id="updateBtn" class="btn btn-primary" onclick="onClickUpdateBtn()"><i class="fas fa-check"></i> 수정</a>
	                	</c:if>
...
//                	<option value="<c:out value="${type1.pk_code}"/>">${type1.code_name}</option>                                
	...
	function regExpTest(){
		let writerEmail = document.querySelector("#writerEmail");
		let phoneNumber = document.querySelector("#writePhone");
		...
		let emailRegex = new RegExp('[a-z0-9]+@[a-z]+\.[a-z]{2,3}');
 		if(writerEmail.value == "" || !emailRegex.test(writerEmail.value)){
			alert("이메일을 확인해주세요");
			writerEmail.focus();
			return false;
		} 
		...
		//전화번호 정규식
		let phone1Exp = /^\d{3}-\d{3,4}-\d{4}$/;
		let phone2Exp = /^\d{2,3}-\d{3,4}-\d{4}$/;
 		if(!phone1Exp.test(phoneNumber.value) && !phone2Exp.test(phoneNumber.value)){
			alert("전화번호를 확인해주세요");
			phoneNumber.focus();
			return false;
		} 
		...
		//serviceType
 		if(document.querySelector("#serviceType").value == "선택하세요"){
			alert("서비스타입을 선택해주세요");
			serviceType.focus();
			return false;
		} 
		...
		//selectType1 select type2
  		if(document.querySelector("#selectType1").value == "선택하세요"){
			alert("문의 유형을 확인해주세요");
			return false;
		} 
 		
  		if(document.querySelector("#selectType2").value == "선택하세요"){
			alert("문의 유형을 확인해주세요");
			return false;
		} 
		...
		//content
		if(document.querySelector("#textContent").value.length == 0){
			alert("내용을 입력하세요");
			return false;
		}
		
		//비미럽ㄴ호
		if(document.querySelector("#board_password").value.length < 4){
			alert("비밀번호를 확인해주세요");
			return false;
		}
